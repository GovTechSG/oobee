...
export interface CfProxyWorker {
  ...
  port: number;
  pacUrl: string; // e.g. https://<worker>.workers.dev/?pac=1&socks-port=8877
  stop: () => Promise<void>;
}

// The PAC script is generated by the Cloudflare Worker (single source of truth
// for the bypass IP list). The client just constructs the URL and hands it to
// Chromium via --proxy-pac-url; Chromium's PAC fetch is a normal HTTP GET that
// happens before proxy config is active, so no chicken-and-egg problem.
function buildPacUrl(workerUrl: string, socksPort: number): string {
  const httpUrl = new URL(workerUrl.replace(/^wss:/i, 'https:').replace(/^ws:/i, 'http:'));
  httpUrl.searchParams.set('pac', '1');
  httpUrl.searchParams.set('socks-port', String(socksPort));
...
export function startCfProxyWorker(): CfProxyWorker | null {
  ...
  const pacUrl = buildPacUrl(workerUrl, port);
  ...
  server.listen(port, '127.0.0.1', () => {
    ...
    consoleLogger.info(`[cfProxyWorker] Chromium PAC URL: ${pacUrl}`);
  });
