...
export const getStoragePath = (randomToken: string): string => {
  ...
  if (!isWritable) {
    if (os.platform() === 'win32') {
      ...
      storagePath = path.join(documentsPath, 'A11y Assist', randomToken);
      ...
      storagePath = path.join(homePath, 'A11y Assist', randomToken);
    }
...
export const getUserDataFilePath = () => {
  ...
  if (platform === 'win32') {
    return path.join(process.env.APPDATA, 'A11y Assist', 'userData.txt');
  }
  if (platform === 'darwin') {
    return path.join(process.env.HOME, 'Library', 'Application Support', 'A11y Assist', 'userData.txt');
  }
  // linux and other OS
  return path.join(process.env.HOME, '.config', 'a11yassist', 'userData.txt');
};
...
// Clean up listeners for process signals (e.g. parent process wants to stop A11y Assist scan mid-point)
...
export const getWcagCriteriaMap = async (
  ...
  disableA11yAssist: boolean = false
...
export const getIssuesPercentage = async (
  ...
  const { totalRulesMustFix, totalRulesGoodToFix, totalRulesMustFixAndGoodToFix } =
    await getTotalRulesCount(enableWcagAaa, disableA11yAssist);
 ...
 * @param disableA11yAssist Whether to disable custom A11y Assist rules
...
export const getPotentialNeedsReviewWcagCriteria = async (
  ...
  // Also check custom A11y Assist rules if they're enabled
 ...
 * @param disableA11yAssist Whether A11y Assist custom rules are disabled
...
export const categorizeWcagCriterion = async (
  ...
  const needsReviewMap = await getPotentialNeedsReviewWcagCriteria(enableWcagAaa, disableA11yAssist);
  if (needsReviewMap[wcagId]) {
    return 'needsReview';
  }
  
  // Get the WCAG criteria map to check the level
  const wcagCriteriaMap = await getWcagCriteriaMap(enableWcagAaa, disableA11yAssist);
...
export const categorizeWcagCriteria = async (
  ...
  const [needsReviewMap, wcagCriteriaMap] = await Promise.all([
    getPotentialNeedsReviewWcagCriteria(enableWcagAaa, disableA11yAssist),
    getWcagCriteriaMap(enableWcagAaa, disableA11yAssist)
  ]);
