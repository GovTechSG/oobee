    ...
    for (const rule of allRules) {
        try {
            ...
            for (const checkId of checks) {
                ...
                const explicitlyReturnsUndefined = evaluateCode.includes('return undefined') || evaluateCode.includes('return;');
        ...
        for (const rule of axeConfig.rules || []) {
            ...
            try {
                const hasIncompleteMessage = rule?.metadata?.messages?.incomplete !== undefined ||
                    (axeConfig.checks || []).some(check => check.id === rule.id && check.metadata?.messages?.incomplete !== undefined);
