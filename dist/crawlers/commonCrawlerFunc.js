...
const buildColorContrastMessage = (node) => {
    ...
    const examples = combos
        .map(example => `foreground ${example.fgColor} on ${example.bgColor} at ${example.fontSize} ${example.fontWeight} text (current contrast ${example.contrastRatio}, expected ${example.expectedContrastRatio})`)
        .join(', and ');
            ...
            if (violation.id === 'target-size') {
                ...
                if (ctx) {
                    ...
                    if (ctx.boxSizing === 'border-box' &&
                        ...
                        const dims = [
                            ...
                            .filter(Boolean)
                            .join(', ');
                        lines.push(`  WARNING: inline style sets ${dims} with box-sizing: border-box — padding is included within those dimensions and will NOT increase the hit area. Fix: remove the explicit width/height and use min-width: 24px; min-height: 24px instead, or place the visual content in a child <span> element.`);
                    }
                    node.failureSummary += '\n' + lines.join('\n');
                }
            }
                ...
                if (ctx) {
                    ...
                    if (ctx.langValue.startsWith('x-')) {
                        lines.push(`  NOTE: "${ctx.langValue}" uses a private-use "x-" prefix. axe-core's valid-lang rule also rejects private-use subtags — you must use a registered IANA language code.`);
                    }
                    if (ctx.textSnippet) {
                        lines.push(`  Element text: "${ctx.textSnippet}". Identify the actual language of this text and use its registered BCP 47 code (e.g., lang="it" Italian, "es" Spanish, "fr" French, "de" German, "zh" Chinese, "ja" Japanese, "ko" Korean, "pt" Portuguese, "ar" Arabic).`);
                    }
                    if (lines.length > 0) {
                        node.failureSummary += '\n' + lines.join('\n');
                    }
